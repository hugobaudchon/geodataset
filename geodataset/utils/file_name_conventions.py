...
def validate_and_convert_product_name(product_name_stem: str):
    standardized_product_name_stem = product_name_stem.replace(" ", "_").replace(
        "-", "_"
    )
    ...
    if not re.match(pattern, standardized_product_name_stem):
        raise ValueError(
            f"The product name stem (without extension) {product_name_stem},"
...
class FileNameConvention(ABC):
    ...
    def create_specifier(
        ...
        voxel_size=None,
        num_points=None,
        outlier_z_thresh=None,
        outlier_k_neighbors=None,
    ):
        if scale_factor is not None:
            ...
            return FileNameConvention.create_specifier(
                scale_factor=1.0,
                voxel_size=voxel_size,
                num_points=num_points,
                outlier_z_thresh=outlier_z_thresh,
                outlier_k_neighbors=outlier_k_neighbors,
            )
        if voxel_size is not None:
            specifier = specifier + f"_vs{str(voxel_size).replace('.', 'p')}"
        elif num_points is not None:
            specifier = specifier + f"_np{str(num_points)}"
        if outlier_z_thresh is not None and outlier_z_thresh > 0:
            specifier = (
                specifier + f"_oz{str(outlier_z_thresh)}_ok{str(outlier_k_neighbors)}"
            )
    ...
    def parse_specifier(specifier):
        ...
        if "oz" in specifier and "ok" in specifier:
            oz_part = specifier.split("oz")[1].split("_")[0]
            outlier_z_thresh = float(oz_part)
            ok_part = specifier.split("ok")[1].split("_")[0]
            outlier_k_neighbors = int(ok_part)

        if scale_factor is None and ground_resolution is None:
            raise ValueError(
                "Specifier must contain either 'sf' for scale factor or 'gr' for ground resolution."
            )
...
class TileNameConvention(FileNameConvention):
    ...
    def _validate_name(name):
        if re.match(TileNameConvention.pattern_aoi_tile_width_height_id, name):
            ...
            raise ValueError(
                f"tile_name {name} does not match any of the supported patterns."
            )
    ...
    def create_name(
        ...
        specifier = FileNameConvention.create_specifier(
            scale_factor=scale_factor, ground_resolution=ground_resolution
        )
    ...
    def parse_name(tile_name: str, return_tile_size=False):
        ...
        if pattern_name == "pattern_aoi_tile_width_height_id":
            int(parts[-1].replace(".tif", ""))
            height = int(parts[-2])
            width = int(parts[-3])
            row = int(parts[-4])
            col = int(parts[-5])
            specifier = parts[-6]
            scale_factor, ground_resolution, _ = FileNameConvention.parse_specifier(
                specifier
            )
...
class CocoNameConvention(FileNameConvention):
    ...
    def _validate_name(name):
        ...
        if not re.match(pattern, name):
            raise ValueError(
                f"coco_name {name} does not match the expected format {pattern}."
            )

    @staticmethod
    def create_name(
        product_name: str, fold: str, scale_factor=None, ground_resolution=None
    ):
        specifier = FileNameConvention.create_specifier(
            scale_factor=scale_factor, ground_resolution=ground_resolution
        )
    ...
    def parse_name(coco_name: str):
        ...
        scale_factor, ground_resolution, _, _, _, _ = (
            FileNameConvention.parse_specifier(specifier)
        )
...
class GeoJsonNameConvention(FileNameConvention):
    ...
    def _validate_name(name):
        ...
        if not re.match(pattern, name):
            raise ValueError(
                f"geojson_name {name} does not match the expected format {pattern}."
            )
...
class GeoPackageNameConvention(FileNameConvention):
    ...
    def _validate_name(name):
        ...
        if not re.match(pattern, name):
            raise ValueError(
                f"geopackage_name {name} does not match the expected format {pattern}."
            )
...
class AoiTilesImageConvention(FileNameConvention):
    ...
    def _validate_name(name):
        ...
        if not re.match(pattern, name):
            raise ValueError(
                f"file_name {name} does not match the expected format {pattern}."
            )
...
class AoiGeoPackageConvention(FileNameConvention):
    ...
    @staticmethod
    def create_name(
        product_name: str, aoi: str, scale_factor=None, ground_resolution=None
    ):
        specifier = FileNameConvention.create_specifier(
            scale_factor=scale_factor, ground_resolution=ground_resolution
        )
...
class PointCloudTileNameConvention(FileNameConvention):
    @staticmethod
    def _validate_name(name):
        pattern = r"^.*_pc_tile_[a-zA-Z0-9]+_((sf|gr)[0-9]+p[0-9]+_)?(vs[0-9]+p[0-9]+_)?(np[0-9]+_)?(oz[0-9]+_ok[0-9]+_)?[0-9]+_[0-9]+(_[0-9]+)?(_[0-9]+)?(_[0-9]+)?\.(ply|las|pcd|laz)$"
        if not re.match(pattern, name):
            raise ValueError(
                f"tile_name {name} does not match the expected format {pattern}."
            )
        else:
            return True
    ...
    def create_name(
        ...
        assert extension in [
            ...
            "las",
            "laz",
        ], f"Extension must be either 'pcd', 'ply', 'las' or 'laz', not {extension}."
        specifier = FileNameConvention.create_specifier(
            scale_factor=scale_factor,
            ground_resolution=ground_resolution,
            voxel_size=voxel_size,
