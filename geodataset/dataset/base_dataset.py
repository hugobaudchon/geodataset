...
class BaseDataset(ABC):
    ...
    Requires the implementation of methods needed for use with PyTorch's DataLoader: __getitem__, __len__ and __iter__.
    """
...
class BaseLabeledRasterCocoDataset(BaseDataset, ABC):
    """
    Abstract class for a dataset that loads COCO datasets and their associated tiles (images). It will recursively
    search for COCO json files and image tiles (.tif, .png...) in the specified root folder and its sub-folders. The
    COCO json files should follow the naming convention defined in the :class:`utils.CocoNameConvention` class. COCO
    jsons generated by this library should automatically follow this convention.
    ...
    def __init__(
        ...
        if not keep_unlabeled or exclude_classes is not None:
            self._filter_unlabeled_excluded_class_tiles(exclude_classes=exclude_classes)
        if few_shot_path is not None:
            skipped_classes = self._filter_tiles_few_shot(few_shot_path)
        else:
            skipped_classes = None
        if skipped_classes is not None or exclude_classes is not None:
            combined_skipped_classes = set()
            if skipped_classes is not None:
                combined_skipped_classes.update(skipped_classes)
            if exclude_classes is not None:
                combined_skipped_classes.update(exclude_classes)
            self._remap_class_ids(list(combined_skipped_classes))

        if len(self.cocos_detected) == 0:
            raise Exception(
                f"No COCO datasets for fold '{self.fold}' were found in the specified root folder."
            )
        elif len(self.cocos_detected) > 0 and len(self.tiles) == 0:
            raise Exception(
                f"Could not find find any tiles associated with the COCO files found."
                f" COCO files found: {self.cocos_detected}."
            )

        print(
            f"Found {len(self.tiles)} tiles and {sum([len(self.tiles[x]['labels']) for x in self.tiles])} labels"
        ...
        for directory in directories:
            for path in directory.iterdir():
                if (
                    ...
                    and ("rgb" in path.name or "orthomosaic" in path.name)
                ):
                    try:
                        product_name, scale_factor, ground_resolution, fold = (
                            CocoNameConvention.parse_name(path.name)
                        )
                        if fold == self.fold:
    ...
    def _reindex_coco_data(self, coco_data: dict):
        ...
        for category in coco_data["categories"]:
            ...
            if category_id not in self.category_id_to_metadata_mapping:
                ...
                category_id in self.category_id_to_metadata_mapping
                and self.category_id_to_metadata_mapping[category_id]["name"]
                != category["name"]
            ):
                raise Exception(
                    f"Category with id {category_id} is duplicated in the COCO dataset,"
    ...
    def _find_tiles_paths(self, directories: List[Path]):
        ...
        for directory in directories:
            tiles_names_to_paths = find_tiles_paths(
                [directory], extensions=self.SUPPORTED_IMG_EXTENSIONS
            )
            ...
            for tile_name, tile_path in tiles_names_to_paths.items():
                if tile_name in self.tiles_path_to_id_mapping:
                    tile_id = self.tiles_path_to_id_mapping[tile_name]
                    if (
                        "path" in self.tiles[tile_id]
                        and self.tiles[tile_id]["path"] != tile_path
                    ):
                        raise Exception(
    ...
    def _remove_tiles_not_found(self):
        ...
        if original_tiles_number != new_tiles_number:
            warn(
                f"Had to remove {original_tiles_number - new_tiles_number} tiles out of {original_tiles_number}"
    ...
    def _filter_unlabeled_excluded_class_tiles(
        self, exclude_classes: Optional[List[int]] = None
    ):
        # Remove tiles without annotation for training
        original_tiles_number = len(self.tiles)
        ...
        for tile_id, tile in self.tiles.items():
            ...
            elif exclude_classes is not None and category_id in exclude_classes:
                tile_ids_excluded.append(tile_id)

        for tile_id in tile_ids_unlabeled:
            del self.tiles_path_to_id_mapping[self.tiles[tile_id]["name"]]
            del self.tiles[tile_id]
        number_unlabeled_removed = len(tile_ids_unlabeled)
        for tile_id in tile_ids_excluded:
            del self.tiles_path_to_id_mapping[self.tiles[tile_id]["name"]]
            del self.tiles[tile_id]
        number_excluded_removed = len(tile_ids_excluded)

        new_tiles_number = len(self.tiles)

        if original_tiles_number != new_tiles_number:
            print(
                f"Had to remove {number_unlabeled_removed} unlabeled tiles and {number_excluded_removed} excluded class tiles out of {original_tiles_number}"
                f" as they are unlabeled or excluded."
            )
            # Need to reindex the tile ids for __getitem__
            self._reindex_tiles()
    ...
    def _filter_tiles_few_shot(self, few_shot_path: str):
        ...
        all_fewshot_filenames = {
            fname for files in fewshot_filenames.values() for fname in files
        }
        ...
        for tile_id, tile in self.tiles.items():
            if self.fold == "train":
                tile_name = tile["name"]
                if not any(
                    fewshot_fname in tile_name
                    for fewshot_fname in all_fewshot_filenames
                ):
    ...
    def _remap_class_ids(self, skipped_classes):
        # Identify all class IDs still present
        ...
        for tile in self.tiles.values():
            for label in tile["labels"]:
                ...
                if remap[old_id] is not None:
                    ...
                    raise ValueError(
                        f"Tile contains skipped class {old_id} after filtering."
                    )
        ...
        for annotation in self.tiles[tile_id]["labels"]:
            for attribute_name in self.other_attributes_names_to_pass:
                if attribute_name in annotation:
                    ...
                    "other_attributes" in annotation
                    and attribute_name in annotation["other_attributes"]
                ):
                    other_attributes[attribute_name].append(
                        annotation["other_attributes"][attribute_name]
                    )
                else:
        ...
        for directory in directories:
            for path in directory.iterdir():
                if (
                    path.is_file()
                    and path.name.endswith(".json")
                    and "pc_coco" in path.name
                ):
                    product_name, scale_factor, ground_resolution, voxel_size, fold = (
                        PointCloudCocoNameConvention.parse_name(path.name)
                    )
                    if fold == self.fold:
                        self._load_coco_json(json_path=path)
                elif (
                    path.is_dir()
                    and path.name != "tiles"
                    and not path.name.startswith("pc_tiles")
                ):
                    self._load_coco_datasets(directories=[path])
